...
protected:
	...
	void init(const int i, const V * __restrict v) {
		if (v)
			new(&value[i]) _V(*v);
		else
			new(&value[i]) _V();
...
public:
	...
	inline bool add(const V * __restrict v, const int s = 1) {
	...
	bool add(int p, const V * __restrict v, const int s = 1) {
		int i;
		if (++p <= 0 || s <= 0 || v == 0)
			return false;
		if (p > this->size) {
...
public:
	...
	bool add(const V * __restrict v, const int s = 1) {
		if (s <= 0 || v == 0)
			return false;
		if (!allocate(size+s))
			return false;
...
protected:
	int * __restrict idx;                 // The index.
	V * __restrict value;                 // Take a guess...
	struct _V {                         // Placement new wrapper
		V _v;
		explicit _V() : _v() {}
		explicit _V(const V & v) : _v(v) {}
...
public:
	...
	inline explicit afset(const int s, const int b = DFLT_BLK,
			const K * __restrict k = 0, const V * __restrict v = 0)
...
protected:
	...
	void init(const int i, const K * __restrict k, const V * __restrict v) {
		new(&key[i]) _K(*k);
		if (v)
			new(&value[i]) _V(*v);
		else
...
public:
	...
	// Add a whole bunch...
	bool add(const K * __restrict k, const V * __restrict v = 0,
			const int s = 1) {
		if (s <= 0 || k == 0)
			return false;
		if (!allocate(this->size+s))
